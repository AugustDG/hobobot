...
void motion_controller_t::update_with_linear_gain(float dt, const std::array<float, 2> &wheel_vels, float override_linear_gain)
{
    update(dt, wheel_vels, override_linear_gain, default_angular_gain, default_theta_gain);
}

void motion_controller_t::update_with_angular_gain(float dt, const std::array<float, 2> &wheel_vels, float override_angular_gain)
{
    update(dt, wheel_vels, default_linear_gain, override_angular_gain, default_theta_gain);
}

void motion_controller_t::update_with_theta_gain(float dt, const std::array<float, 2> &wheel_vels, float override_theta_gain)
{
    update(dt, wheel_vels, default_linear_gain, default_angular_gain, override_theta_gain);
}

void motion_controller_t::update(float dt, const std::array<float, 2> &wheel_vels, float override_linear_gain, float override_angular_gain, float override_theta_gain)
...
const std::array<float, 2> &motion_controller_t::get_target_wheel_vels() const
{
    return target_wheel_vels;
}
