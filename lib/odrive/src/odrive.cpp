...
bool wait_for_heartbeat_all(uint32_t timeout) { return wait_for_heartbeat(odrives, timeout); }
...
void set_position_all(
  ...
  if (positions.size() != odrives.size() || ff_velocities.size() != odrives.size() ||
      ff_torques.size() != odrives.size()) {
...
void odrive_t::set_idle() { set_state(ODriveAxisState::AXIS_STATE_IDLE); }

void odrive_t::set_closed_loop_control() { set_state(ODriveAxisState::AXIS_STATE_CLOSED_LOOP_CONTROL); }
